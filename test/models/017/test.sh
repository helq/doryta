...
# against GoL with only positive synaptic weighths and NO leak
exec mpirun -np 1 "$doryta" --spike-driven --output=output/gol-20x20-no-leak \
    --load-model="$modelsdir"/gol/snn-models/gol-20x20-no-leak.doryta.bin \
    --load-spikes="$modelsdir"/gol/spikes/20x20/gol-die-hard.bin \
