...
int main(int argc, char *argv[]) {
    ...
    char const fmt[] = "output/conv2d-nostrides-final-state-gid=%lu.txt";
    ...
    // Defining neurons
    // IN (conv2d)
    layout_master_neurons(400, 0, last_node);  // 20 x 20
    // OUT (conv2d)
    layout_master_neurons(320, 0, last_node); // 20 x 16
    ...
    layout_master_synapses_conv2d(0, 399, 400, 719,  &conv2d_params);
    ...
    probes_firing_init(5000, "output", "conv2d-nostrides", false);
    probes_lif_voltages_init(5000, "output", "conv2d-nostrides");
